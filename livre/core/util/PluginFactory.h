 ...
 *
 * The PluginFactory selects the a plugin for a given arguments.
 *
 ...
 *
 * Example: @include tests/core/pluginFactory.cpp
 */
template< class PluginT, class... Args >
class PluginFactory
{
public:
    typedef Plugin< PluginT, Args... > PluginHolder;
     ...
     */
    PluginT* create( Args&&... initData );

    /** Register a plugin type. */
    void register_( const Plugin< PluginT, Args... >& plugin );

    /** Deregister a plugin type. */
    bool deregister( const Plugin< PluginT, Args... >& plugin );
     ...
     */
    lunchbox::DSOs load( const int version, const std::string& path,
               const std::string& pattern );
    lunchbox::DSOs load( const int version, const Strings& paths,
               const std::string& pattern );
...
private:
    Plugins _plugins;
    typedef std::unordered_map< lunchbox::DSO*, PluginHolder > PluginMap;
    PluginMap _libraries;

    void _load( lunchbox::DSOs& result, const int version, const std::string& path,
                const std::string& pattern );
